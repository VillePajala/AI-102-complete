...
Define tools using JSON Schema function parameters, implement tool_choice options (auto, required, none), and build the tool-call-response loop pattern.
...
Explore the routing agent pattern for task delegation, agent handoff and context passing, and Foundry Agent Service multi-agent orchestration.
...
Deep-dive into code interpreter capabilities and sandbox, file search with vector stores, and the Assistants API thread and run lifecycle.
